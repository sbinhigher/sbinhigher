...
```
2년차 데이터 분석가, 고성빈입니다.
데이터 분석가로서 2년 간 일하면서 비지니스 문제를 구조화하고, 분석-모델링-검증-리포팅까지 수행하며 의사결정에 기여했습니다.
그 과정에서 PYTHON 기반의 비정형 데이터 분석(이미지・텍스트)과 ML 모델링 역량을 프로젝트 중심으로 쌓았습니다.
학과 연구・개인 프로젝트로 관광객 리뷰 토픽 모델링과 군집화 분석을 활용해서 소비자와 관광객 행동 데이터 분석을 진행했습니다.
GA4와 LOOKER STUDIO 기반으로 지표 정의 및 리포팅/대시보드 구성을 실습했고, SQL·TABLEAU 활용 역량을 꾸준히 확장 중입니다.
위 경험을 바탕으로 사용자/고객 행동 데이터를 지표로 정의하고, 실행 가능한 인사이트로 연결하는 분석가 역할에 기여하겠습니다.
```
<br>

### 핵심 역량

#### 1️⃣ Python을 이용한 데이터 분석 역량
- EDA/전처리: pandas·numpy·scikit-learn으로 결측/이상치 처리, 피처 가공, 통계 요약
- 시각화: matplotlib·plotly(또는 seaborn)로 탐색형 그래프/리포트 초안 제작
- 머신러닝: RandomForest·LightGBM 등 분류/예측 베이스라인 구현 및 비교
- 비정형 분석: 텍스트 크롤링(Requests, Selenium, Playwright) 구현, 토픽/감성 분석(BERTopic 등)을 통한 리뷰 분석

#### 2️⃣ SQL을 활용한 데이터 추출·KPI 산출 역량
- RDB 쿼리: PostgreSQL·MySQL에서 조인/윈도우/CTE 활용 가능
- KPI 산출: 코호트·리텐션·재구매율·이탈률 등 이커머스 핵심 지표 계산
- 코딩테스트 대응: Programmers/LeetCode 기반 중급 난도 SQL 풀이 경험

#### 3️⃣ BI 시각화·리포팅 역량
- 도구: Tableau 초급, 기본 차트·필터·간단한 파라미터를 활용한 프로토타입 대시보드 작성
- 활용 범위: 정식 운영 대시보드보다는 탐색·공유용 초안 제작과 결과 스냅샷 보고에 초점
- 자료 연계: 수업/실습 산출물을 리포트 형태로 정리

#### 4️⃣ 제품/도메인 분석 역량(이커머스·교통·관광)
- 이커머스: 장바구니·전환·재구매 관점의 가설 수립 및 검증, 예측 모델 도입하여 해석력 강화
- 교통/공공: 도로 위험도 예측 등 도메인 KPI 정의·데이터 수집·의사결정 지원
- 관광: 리뷰/텍스트 기반 트렌드 분석(SNA/감성/토픽)과 정책/서비스 시사점 도출

#### 5️⃣ 컴퓨터비전·이미지 데이터 파이프라인 역량
- 이미지 전처리: OpenCV·Pillow로 리사이즈/증강/형식 변환(json2txt 등) 자동화
- 객체탐지: YOLO 계열 모델 실험 및 라벨링 워크플로우 운영(메타데이터 추출 포함)
- 현장 적용: 풍력발전기 블레이드 결함 탐지, 차량 데미지 탐지 등 B2B 프로젝트 협업

#### 6️⃣ 데이터 파이프라인·협업 역량
- 파이프라인: 크롤링(requests/Playwright)  저장  전처리  분석/리포팅 단일 흐름 설계
- 협업: 개발자/DBA와 역할 분담, 요구사항 정리, 산출물 표준화
- 문서화: GitHub/Notion 기반 산출물 정리 및 재현 가이드 제공

#### 7️⃣ 교육/콘텐츠 제작 역량(강의 레포지토리 기반)
- 커리큘럼 운영: 제주대학교 관광경영학과 학생 대상
- 실습 설계: 라이브러리 데모 모듈, 과제/평가 파일 구조, 정답/해설 분리
- 버전관리: 강의 자료 변경 이력 관리와 배포
---
...
| 기간              | 형태   | 직급    | 소속             |
|------------------|--------|---------|------------------|
| 2025.09 – 2026.02 | 계약직 | 연구원 | 제주대학교 관광경영학과 |

##### 📌 주요 프로젝트

- **RISE산학공동연구(R&D) | AI융합 실감형 혁신관광 콘텐츠 개발**  
  ⏱ *2025.09 – 2026.02*  
- **BERTopic 기반 Google maps 리뷰 주제·감성 분석**  
  ⏱ *2025.09 – 2026.02*  
